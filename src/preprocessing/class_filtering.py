...
def filter_classes(target_class_ids: list[int]):
    # load the original GeoJSON file
    geojson_data = load_annotations_master()
    ...
    write_to_annotations_filtered(filtered_features)
    # map target class ids to class names for YOLO data config
    target_class_names = [get_class_name_from_id(id_key) for id_key in target_class_ids]
    # update YOLO data config
    update_data_config_class_names(target_class_names)
    update_data_config_class_count(len(target_class_names)) 
